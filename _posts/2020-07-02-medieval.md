...
For very small $$s$$, we have $$r<<1$$ and $$k_{s}(N)=b /(1-s b) \cong b$$, which means that all interactions stay essentially limited to the households **and there is no city as a network of socio-economic relations at all!** In contrast, for $$s$$ close to 1, we have $$r>1$$, and  we can write:
...
Let's plot buil-up area vs. population for all cities in the dataset to begin with:
