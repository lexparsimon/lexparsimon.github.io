...
Let's say that a central planner can prescribe each agent a feasible action. Thus, the planner selects any element of the Cartesian product set. Let $$\boldsymbol{w} \equiv\left(w_{j}, j \in \boldsymbol{J}\right)$$ be any set of assigned actions with finite mean $$\mu_{w} \equiv \int w_{j} d P(j)$$. Then, Jensen’s inequality yields
